...
function TB_OnEvent(event)
	...
	elseif (event == "VARIABLES_LOADED") then
		...
		if (TBSettings[TBSettingsCharRealm].Announcements[TB_GUI_FF]) then
			if (TBSettings[TBSettingsCharRealm].Announcements[TB_GUI_FF]["Text"] == 1) then
				TBSettings[TBSettingsCharRealm].Announcements[TB_GUI_FF]["Text"] = TB_defaultText_FF;				-- Default value for text_g
			end
		end
